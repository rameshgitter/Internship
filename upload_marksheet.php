        ...
        if (strtolower($file_info['extension']) !== 'pdf') {
            ...
            try {
                ...
                if (move_uploaded_file($uploaded_file['tmp_name'], $file_path)) {
                    ...
                    // Insert marks data into student_marks
                    if ($extracted_data && !empty($extracted_data['subjects'])) {
                        foreach ($extracted_data['subjects'] as $subject) {
                            $stmt = $pdo->prepare("
                                INSERT INTO student_marks (student_id, subject_code, subject_name, credit, letter_grade, total_grade_point, semester, academic_year, uploaded_by, upload_date)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
                        ...
                        $stmt = $pdo->prepare("
                            INSERT INTO student_marks (student_id, semester, academic_year, uploaded_by, upload_date)
                            VALUES (?, ?, ?, ?, NOW())
                        ");
                        $stmt->execute([
                            $student_id,
